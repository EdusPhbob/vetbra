...
export default function AdminCrmvModerationPage() {
  ...
  const handleVetAction = async (
    ...
    acao: 'SUSPENDER' | 'REATIVAR' | 'BLOQUEAR' | 'DESBLOQUEAR' | 'EXCLUIR' | 'LIBERAR_TRIAL' | 'CONFIRMAR_PAGAMENTO',
    extra?: any
  ) => {
    if (acao === 'EXCLUIR') {
  ...
  const filteredVets = vets.filter(v => {
    const assinatura = v.assinaturas?.[0];
    const fatura = assinatura?.faturas?.[0];
    const isPago = fatura?.status === 'PAGA' || assinatura?.status === 'ATIVA';
    ...
    else if (statusFilter === 'SUSPENSO') matchesFilter = v.statusGeral === 'SUSPENSO';
    else if (statusFilter === 'BLOQUEADO') matchesFilter = v.user?.ativo === false;
    else matchesFilter = v.crmvStatus === statusFilter;
  ...
  const totalNaoPagos = vets.filter(v => {
    const ass = v.assinaturas?.[0];
    const fat = ass?.faturas?.[0];
    return fat?.status !== 'PAGA' && ass?.status !== 'ATIVA' && ass?.status !== 'TRIAL';
  }).length;
  const totalEmTrial = vets.filter(v => v.assinaturas?.[0]?.status === 'TRIAL').length;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <div className="px-3.5 py-2 bg-rose-50 border border-rose-200 rounded-2xl text-center">
                <span className="text-[10px] font-bold text-rose-600 uppercase tracking-wider block">Não Pagos (Pix)</span>
                <span className="text-lg font-black text-rose-800">{totalNaoPagos}</span>
              </div>
              <div className="px-3.5 py-2 bg-indigo-50 border border-indigo-200 rounded-2xl text-center">
                <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider block">Em Teste (Trial 7D)</span>
                <span className="text-lg font-black text-indigo-800">{totalEmTrial}</span>
              </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {filteredVets.map((vet) => {
                  ...
                  const assinatura = vet.assinaturas?.[0];
                  const fatura = assinatura?.faturas?.[0];
                  const isPago = fatura?.status === 'PAGA' || assinatura?.status === 'ATIVA';
                  const isTrial = assinatura?.status === 'TRIAL';
                  const diasRestantesTrial = (isTrial && assinatura?.dataFimPeriodo)
                    ...
                    >
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {isNovo && (
                                <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase bg-purple-100 text-purple-900 border border-purple-300 flex items-center gap-1">
                                  <Sparkles className="w-3 h-3 text-purple-600" /> Novo
                                </span>
                              )}
                              ...
                              {isPago ? (
                                ...
                                <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase bg-rose-100 text-rose-900 border border-rose-300 flex items-center gap-1 animate-pulse">
                                  <AlertCircle className="w-3 h-3 text-rose-600" /> Aguardando Pix (Não Pago)
                                </span>
                              )}
                      ...
                      <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {/* Conceder Teste Grátis de 7 dias */}
                          {!isPago && !isTrial && (
                            <button
                              onClick={() => handleVetAction(vet.id, 'LIBERAR_TRIAL', { dias: 7 })}
                              disabled={processingActionId === vet.id}
                              className="px-3.5 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-800 border border-indigo-300 text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
                              title="Concede 7 dias de acesso e teste gratuito para este médico"
                            >
                              <Gift className="w-3.5 h-3.5 text-indigo-600" />
                              Liberar Teste (7 dias)
                            </button>
                          )}
