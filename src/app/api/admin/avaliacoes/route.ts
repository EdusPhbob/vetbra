...
export async function GET() {
  try {
    // 1. Busca todas as avaliações com dados do profissional
    ...
    // 2. Busca todos os veterinários para compilar o ranking comparativo
    ...
    // 3. Monta as métricas de reputação para cada veterinário
    const ranking = todosVets.map(v => {
      ...
      return {
        ...
        slug: v.slug,
        crmvNumero: v.crmvNumero,
        crmvUf: v.crmvUf,
        fotoPerfilUrl: v.fotoPerfilUrl,
        nomeSocialOuClinica: v.nomeSocialOuClinica,
        statusGeral: v.statusGeral,
    ...
    // Melhores avaliados (com pelo menos 1 avaliação, ordenados por maior média e maior volume)
    ...
    // Veterinários com mais reclamações (ordenados por volume de notas 1 e 2)
...
export async function PATCH(request: Request) {
  try {
    ...
    const { avaliacaoId, novoStatus, motivo } = body;

    if (!avaliacaoId || !novoStatus) {
      return NextResponse.json({ error: 'Informe o ID da avaliação e o novo status.' }, { status: 400 });
    }
    ...
    const atualizada = await prisma.avaliacao.update({
      where: { id: avaliacaoId },
      data: { status: novoStatus as AvaliacaoStatus }
    });
